...
export default function FamilyDashboard({ user, setPage }: FamilyDashboardProps) {
  ...
  const professionals: Professional[] = [
    {
      ...
      available: true,
      verified: true,
      image: 'https://images.unsplash.com/photo-612349317150-e413f6a5b16d?auto=format&fit=crop&q=80&w=150&h=150',
  ...
  const userName = user?.full_name || user?.fullName || 'Familia';
  const userPlan = user?.plan || 'Básico';
  const userPhoto = user?.photo_url || user?.photoUrl;
